...
from geom_filter_edges import FilterEdges
...
class TopKPooling(torch.nn.Module):
    ...
    def forward(
        ...
        edge_index_map = torch.arange(edge_index.size(1), device=edge_index.device)
        connect_out, perm_edges = self.connect(select_out, edge_index, edge_attr, batch, edge_index_map)

        # perm_edges = connect_out.perm_edge_map

        return (x, connect_out.edge_index, connect_out.edge_attr,
                connect_out.batch, perm, perm_edges)
