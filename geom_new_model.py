...
class NodeModel(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        # out_sum = scatter_add(edge_attr, dest, dim=0, dim_size=x.size(0))
        # out_sum =torch.zeros(x.size(0), edge_attr.size(1), device=edge_attr.device)
        # out_sum.torch.scatter_reduce_(dim=0, index=dest, src=edge_attr, reduce='mean') #, dim_size=x.size(0))
...
class GNN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, node_positions):
        ...
        if self.plot: 
            plot_graph_positions(node_positions, 'Initial Graph', f'../plot_{i}.png')
            i+=1
            plot_graph(node_positions, edge_index, 'Graph', f'../plot_{i}.png')
            i+=1
